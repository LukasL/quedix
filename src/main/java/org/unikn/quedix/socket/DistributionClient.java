 ...
 */
public class DistributionClient extends SocketClient implements
		org.unikn.quedix.core.Distributor {
	 ...
	 *            Collection name.
	 * @return <code>true</code> if collection exists, <code>false</code>
	 *         otherwise.
	 */
	private boolean checkCollectionExistence(final BaseXClient client,
	 ...
	 */
	public void distributeXml(final BaseXClient client, final String name,
			final BufferedInputStream bis, final File file) throws IOException {
	 ...
	 *             Exception while writing with client.
	 */
	private void distributeXmlChunked(final BaseXClient client,
			final String name, final BufferedReader br, final File file,
	 ...
	 *             Exception occurred.
	 */
	public long distributeRoundRobin(final File dir, final String name,
			final String[] serverIds) throws IOException {
	 ...
	 *             Exception occurred.
	 */
	public long distributeRoundRobinChunked(final File dir, final String name,
			final String[] serverIds) throws IOException {
		File[] files = dir.listFiles();
		long count = 0;
		for (File file : files) {
	 ...
	 *             Exception occurred.
	 */
	public long distributeAdvanced(final File dir, final String name,
			final String[] serverIds) throws IOException {
		File[] files = dir.listFiles();
		long count = 0;
		for (File file : files) {
	 ...
	 *             Exception occurred.
	 */
	public long distributePartitioned(final File dir, final String name,
			final String[] serverIds) throws IOException {
		File[] files = dir.listFiles();
		long count = 0;
		for (File file : files) {
	...
	private void buildRefactoringString() throws IOException {
		...
		BufferedReader br = new BufferedReader(new InputStreamReader(this
				.getClass().getResourceAsStream("/" + RQ)));
		String l;
		while ((l = br.readLine()) != null) {
			sb.append(l + "\n");
		}
