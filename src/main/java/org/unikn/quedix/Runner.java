...
public class Runner {
	...
	public static void main(final String[] args) throws IOException,
		...
		if (a == null)
			...
			String algo = a.getPar().get(Arg.Paramter.ALGO);
			...
			case DISTRIBUTION_REST:
				run.distributeCollection(a.getPar().get(Arg.Paramter.INPUT), a
						.getPar().get(Arg.Paramter.NAME), ClientType.REST, ag);
				break;
			case DISTRIBUTION_SOCKETS:
				run.distributeCollection(a.getPar().get(Arg.Paramter.INPUT), a
						.getPar().get(Arg.Paramter.NAME), ClientType.SOCKETS,
						ag);
				break;

			case MAP_REST:
				run.executeMap(a.getPar().get(Arg.Paramter.MAP),
						ClientType.REST);
				break;
			case MAP_SOCKETS:
				run.executeMap(a.getPar().get(Arg.Paramter.MAP),
			...
			case MAP_AND_REDUCE_REST:
				run.executeMapReduce(a.getPar().get(Arg.Paramter.MAP), a
						.getPar().get(Arg.Paramter.REDUCE), ClientType.REST);
				break;

			case MAP_AND_REDUCE_SOCKETS:
				run.executeMapReduce(a.getPar().get(Arg.Paramter.MAP), a
						.getPar().get(Arg.Paramter.REDUCE), ClientType.SOCKETS);
				break;
	 ...
	 *            Name for collection.
	 * @param type
	 *            Type, either REST or sockets.
	 * @param algo
	 *            Distribution algorithm.
	 * @throws IOException
	 *             Exception occurred.
	 */
	public void distributeCollection(final String xmlDir, final String name,
	 ...
	 *             XQ file not found.
	 */
	public void executeMap(final String xq, final ClientType type)
			throws IOException {
		if (type == ClientType.REST)
			map(new MapClient(new RestClient(initHttpDataServersMonds(),
					new MetaData()), new File(xq)));
		else {
	 ...
	 *             Query exception.
	 */
	public void executeMapReduce(final String mapXq, final String reduceXq,
	...
	public Map<String, String> initHttpDataServersMonds() {
		Map<String, String> dataServers = new HashMap<String, String>();
		// dataServers.put(HTTP + MOND_1 + COLON + MOND_HTTP_PORT + SLASH,
		// REST_COL);
		dataServers.put(HTTP + MOND_2 + COLON + MOND_HTTP_PORT + SLASH,
				REST_COL);
	...
	public Map<String, BaseXClient> initBaseXClientsMonds() throws IOException {
		Map<String, BaseXClient> clients = new HashMap<String, BaseXClient>();
		// clients.put("mond01", new BaseXClient(MOND_1, MOND_SOCKET_PORT, USER,
		// PW));
		clients.put("mond02", new BaseXClient(MOND_2, MOND_SOCKET_PORT, USER,
				PW));
	...
	private static Arg parseArgs(final String[] args) {
		...
		for (int i = 0; i < args.length; i++) {
			...
			} else if (args[i].equals("-rrs")) {
				// algorithm
				params.put(Arg.Paramter.ALGO,
						DistributionAlgorithm.ROUND_ROBIN_SIMPLE.name());

			} else if (args[i].equals("-rrc")) {
				// algorithm
				params.put(Arg.Paramter.ALGO,
						DistributionAlgorithm.ROUND_ROBIN_CHUNK.name());

			} else if (args[i].equals("-as")) {
				// algorithm
				params.put(Arg.Paramter.ALGO,
						DistributionAlgorithm.ADVANCED.name());

			} else if (args[i].equals("-ac")) {
				// algorithm
				params.put(Arg.Paramter.ALGO,
						DistributionAlgorithm.ADVANCED_CHUNK.name());

			} else if (args[i].equals("-p")) {
				// algorithm
				params.put(Arg.Paramter.ALGO,
						DistributionAlgorithm.PARTITIONING.name());
		...
		if (params.containsKey(Arg.Paramter.INPUT)
				&& params.containsKey(Arg.Paramter.NAME)) {
			type = isRest ? StartType.DISTRIBUTION_REST
					: StartType.DISTRIBUTION_SOCKETS;
		} else if (params.containsKey(Arg.Paramter.MAP)
				&& !params.containsKey(Arg.Paramter.REDUCE)) {
			type = isRest ? StartType.MAP_REST : StartType.MAP_SOCKETS;
		} else if (params.containsKey(Arg.Paramter.MAP)
				&& params.containsKey(Arg.Paramter.REDUCE)) {
			type = isRest ? StartType.MAP_AND_REDUCE_REST
					: StartType.MAP_AND_REDUCE_SOCKETS;
		}
