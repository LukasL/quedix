...
public class Runner {
        ...
        if (a == null)
            ...
            case DISTRIBUTION_REST:
                run.distributeCollection(a.getPar().get(Arg.Paramter.INPUT), a.getPar()
                    .get(Arg.Paramter.NAME), ClientType.REST, DistributionAlgorithm.valueOf(a.getPar().get(
                    Arg.Paramter.ALGO)));
                break;
            case DISTRIBUTION_SOCKETS:
                run.distributeCollection(a.getPar().get(Arg.Paramter.INPUT), a.getPar()
                    .get(Arg.Paramter.NAME), ClientType.SOCKETS, DistributionAlgorithm.valueOf(a.getPar()
                    .get(Arg.Paramter.ALGO)));
                break;
    ...
    public void distributeCollection(final String xmlDir, final String name, final ClientType type,
        final DistributionAlgorithm algo) throws IOException {
        try {
            Client cl;
    ...
    private static Arg parseArgs(final String[] args) {
        ...
        for (int i = 0; i < args.length; i++) {
            if (args[i].equals("-m")) {
                ...
                // algorithm
                params.put(Arg.Paramter.ALGO, DistributionAlgorithm.ROUND_ROBIN_SIMPLE.name());

            } else if (args[i].equals("-rrc")) {
                // algorithm
                params.put(Arg.Paramter.ALGO, DistributionAlgorithm.ROUND_ROBIN_CHUNK.name());

            } else if (args[i].equals("-as")) {
                // algorithm
                params.put(Arg.Paramter.ALGO, DistributionAlgorithm.ADVANCED.name());

            } else if (args[i].equals("-ac")) {
                // algorithm
                params.put(Arg.Paramter.ALGO, DistributionAlgorithm.ADVANCED_CHUNK.name());

            } else if (args[i].equals("-p")) {
                // algorithm
                params.put(Arg.Paramter.ALGO, DistributionAlgorithm.PARTITIONING.name());
